...
\|\mathbf{X}\|_{\bm{w},*}.
\end{equation}
where $\bm{w}$ is the weight vector for singular values of $\mathbf{X}$. In In \cite{wnnmijcv} and this paper, $\bm{w}$ is set as $w_{i}^{k+1}=\frac{C}{|\sigma_{i}(\mathbf{X}_{k})|+\epsilon }$ where $\epsilon>0$ is a small number to avoid zero numerator. Note that if $\sigma_{r}=\sigma_{g}=\sigma_{b}$, the proposed MC-WNNM model will be reduced to the concatenated WNNM model as a special case.
