...
Numerous methods \cite{nlm,ksvd,bm3d,epll,lssc,wnnm,pgpd,mlp,csf,chen2015learning} has been proposed for grayscale image denoising task. These methods could be employed to denoise each channel in color images separately. However, the performance would be largely reduced when compared to the performance of these methods on grayscale images \cite{mairal2008sparse}. This could be alleviated if we denoise each image patch of R, G, B channels as a long concatenated vector. However, this simple modification treat each channel equally and ignores the differences among these channels. To explore the difference and correlation among the three channels in color images, several methods \cite{crosschannel2016,Liu2008,almapg,Zhu_2016_CVPR,noiseclinic,ncwebsite,neatimage} have also been proposed with different strategies for color image denoising task.  

In this paper, we propose a new strategy for color image denoising. Specifically, we employ different weights for the R, G, B channels to deal with the distinct noise property among these channels. Experiments will show that this new 
