...
There are several work  \cite{cbm3d,mairal2008sparse,almapg,noiseclinic,crosschannel2016,Zhu_2016_CVPR} proposed specifically for color image denoising. These methods either treat equally the R, G, B channels in color images \cite{cbm3d,mairal2008sparse,almapg} or ignore the non-local self similarity property of natural images \cite{noiseclinic,crosschannel2016,Zhu_2016_CVPR}. On one hand, since the noise structures and levels among the R, G, B channels in color images are quite different due to the processing steps, such as demosaicing, white balance, etc., of in-camera imaging pipelines \cite{karaimer_brown_ECCV_2016}, the methods treating each channel equally would generate false colors or artifacts \cite{mairal2008sparse}. In order to deal with the R, G, B channels in color images more effectively, different noise properties of these channels should be considered in denoising color images, especially real noisy images. On the other hand, the performance of existing denoising methods would be largely depressed if the non-local self similarity property of natural images is ignored \cite{bm3d,wnnm}. This can be validated by the success of the state-of-the-art denoising methods \cite{bm3d,lssc,wnnm,pgpd} which all exployed the non-local self similarity (NSS) property of natural images in different ways.

Recently, the weighted nuclear norm minimization method (WNNM) \cite{wnnm} is among the most successful methods on image denoising task which employ the NSS property o f natural images. In this paper, we proposed a noval multi-channel WNNM model for color image denoising by intorducing a weighting matrix to the WNNM method. However, if we add a weighting matrix $\mathbf{W}$ to the WNNM model, the resulting multi-channel WNNM model no longer has closed-form solutions as WNNM. This makes the proposed model more challenging to solve. To solve this problem, we reformulate the proposed multi-channel WNNM problem into a linear constrained non-convex program with two variables. And the relaxed problem can be solved by employing the alternating direction method of multipliers (ADMM) \cite{admm}. Each variable can be updated with closed-form solution \cite{wnnm,lugsvt}. The convergency resutls with detailed proof are given to guarantee a rational termination of the proposed model. 

\section{Related Work}

\subsection{Nuclear Norm Minimization}
As the tightest convex surrogate function of the matrix rank minimization \cite{Guaranteed,fazelPhDthesis}, the nuclear norm minimization (NNM) problem has been extensive studied in low rank matrix approximation (LRMA) \cite{srebro2003weighted,cai2010singular,candes2011robust,lin2011linearized}. A standard NNM problem is as follows:
\vspace{-3mm}
\begin{equation}
\vspace{-3mm}
\mathbf{\hat{X}}
...
\end{equation}
The original NNM model treats equally all the singular values but ignores their different importance \cite{wnnm}. To make it more flexible, the NNM model  has been extended by the truncated nuclear norm minimization model \cite{tnnm}, the partial sum minimization of singular values \cite{PartialSum}, and the weighted nuclear norm minimization (WNNM) model \cite{wnnm}, etc. 

Among these models, the WNNM model
\vspace{-3mm}
\begin{equation}
\vspace{-3mm}
...
\|\mathbf{X}\|_{\bm{w},*}
\end{equation}
adds weights to each singular values, i.e., $\|\mathbf{X}\|_{\bm{w},*}=\sum_{i}w_{i}\sigma_{i}(\mathbf{X})$ is the weighted nuclear norm of matrix $\mathbf{X}$, where $\bm{w}=[w_{1},...,w_{n}]^{\top}, w_{i}\ge 0$ is the weight vector.\ According to the Remark 1 of \cite{wnnmijcv}, the problem (4) has closed-form solution if the weights are in a non-decreasing order. Though having achieved excallent performance on grayscale image denoising task, directly extending the WNNM model on color image denoising, by processing each channel seperately or joint vectors concatenated by multiple channels, would generate false colors or artifacts \cite{mairal2008sparse}. In this paper, we proposed a multi-channel WNNM model which can deal with color images and preserve the power of WNNM on exploring the non-local self similarity property of natural images.
