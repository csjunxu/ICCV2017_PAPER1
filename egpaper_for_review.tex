...
Image denoising is an important problem in enhancing the image quality in computer vision systems. It aims to recover the clean image $\mathbf{x}$ from the noisy observation $\mathbf{y}=\mathbf{x}+\mathbf{n}$, where $\mathbf{n}$ is often assumed to be additive white Gaussian noise (AWGN). Most image denoising methods \cite{nlm,bm3d,foe,ksvd,epll,lssc,ncsr,pgpd,wnnm,mlp,csf,chen2015learning} are designed for grayscale (i.e., single channel) images. Though these methods have achieved excellant denoising performance on grayscale images, directly extending these methods to process each channel of color images seperately would generate false colors or artifacts \cite{mairal2008sparse}.

When compared to the grayscale image denoising problems, little attention has been paid to their color counterpairs. There are several work \cite{cbm3d,mairal2008sparse,almapg,noiseclinic,crosschannel2016,Zhu_2016_CVPR} proposed specifically for color images. These methods either treat equally the R, G, B channels in color images \cite{cbm3d,mairal2008sparse,almapg} or fail to explore the properties of natural images \cite{noiseclinic,crosschannel2016,Zhu_2016_CVPR}. On one hand, treating each channel equally in color images is not that reasonable, since the noise structures and levels among the R, G, B channels are different in natural images due to the on board processing in in-camera imaging pipelines \cite{karaimer_brown_ECCV_2016}. Hence, the methods treating each channel equally would generate fake colors or artifacts \cite{mairal2008sparse}. On the other hand, the performance of denoising methods would be largely depressed if the non-local self similarity property of natural images has not been fully exploited \cite{wnnm}. Due to the two reasons, some important information in color images would be highly degraded when applying these denoising methods directly on color images. 
