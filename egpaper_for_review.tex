...
In Fig.\ \ref{fa}, we show the denoising results of WNNM \cite{wnnm} on a color image from the Kodak PhotoCD Dataset with synthetic additive white Gaussian noise. The noise added to the R, G, B channels are of standard derivations with $\sigma=40, 20, 30$.\ One can see that the WNNM method would se two methods do not perform well on this noisy image either. 
