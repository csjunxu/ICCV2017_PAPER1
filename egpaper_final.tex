...
In Algorithm 1 for solving the MC-WNNM model via ADMM, the cost for updating $\bm{X}$ is $\mathcal{O}(\max(p^{4}M, M^{3}))$, while the cost for updating $\bm{Z}$ is $\mathcal{O}(p^{4}M+M^{3})$.\ The costs for updating $\bm{A}$ and $\rho$ can be ignored when compared to the updating of $\bm{X}$ and $\bm{Z}$.\ So the overall complexity is $\mathcal{O}((p^{4}M+M^{3})K_{1})$, where $K_{1}$ is the number of iterations.\ In Algorithm 2 for image denoising, we need further consider the number of image patches $N$ extracted from the input noisy image and the number of itertations $K_{2}$ and ignore the cost for searching similar patches.\ The overall cost is $\mathcal{O}((p^{4}M+M^{3})K_{1}K_{2}N)$.
...
\vspace{-1.5mm}
\caption{Denoised images of the real noisy image ``Dog'' \cite{ncwebsite} by different methods.\ The estimated noise levels of RGB channels are (R), (G), and (B), respectively.\ The images are better to be zoomed in on screen.}
\label{f3}
\vspace{-4mm}
\end{figure*}
...
\begin{table*}
\vspace{-4mm}
\caption{PSNR(dB) results and computational time of different methods on 15 cropped real noisy images used in \cite{crosschannel2016}.}
\label{t2}
\label{tabb}
\begin{center}
...
\caption{Denoised images of a region cropped from the real noisy image ``Canon 5D Mark 3 ISO 3200 1" \cite{crosschannel2016} by different methods.\ The estimated noise levels of RGB channels are (R), (G), and (B), respectively.\ The images are better to be zoomed in on screen.}
\vspace{-5mm}
\label{f4}
\end{figure*}
